...
TRI_REGION_ROPE_REBASE="${TRI_REGION_ROPE_REBASE:-0}"
...
if [ "$FIXED_SINK_ROPE_REBASE" = "1" ] || [ "$FIXED_SINK_ROPE_REBASE" = "true" ] || [ "$FIXED_SINK_ROPE_REBASE" = "True" ]; then
  SINK_REBASE_ARGS="--fixed_sink_rope_rebase $SINK_REBASE_ARGS"
fi
...
echo "  Fixed sink RoPE rebase: enabled=$FIXED_SINK_ROPE_REBASE"
